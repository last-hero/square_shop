...
class SSImport {	
	...
	*/
	public static function importSamples(){
		global $REX;
		$query = array();
		$files = array('category.sql', 'article.sql', 'client.sql');
		...
		// Order einfügen
		$query = 'SELECT c.id as client_id, NOW() as no, NOW() as date , c.title, c.firstname, c.lastname, c.street, c.zip, c.city, c.telephone, c.email, c.title as d_title, c.firstname as d_firstname, c.lastname as d_lastname, c.street as d_street, c.zip as d_zip, c.city as d_city, c.telephone as d_telephone, c.email as d_email, NOW() as createdate FROM rex_square_shop_client c LIMIT 30';
		$res = SSDBSQL::executeSql($query, false);
		
			$query = 'INSERT INTO `rex_square_shop_order` (`client_id`, no, date, billing_title, billing_firstname, billing_lastname, billing_street, billing_zip, billing_city, billing_telephone, billing_email, delivery_title, delivery_firstname, delivery_lastname, delivery_street, delivery_zip, delivery_city, delivery_telephone, delivery_email,createdate) VALUES ';
